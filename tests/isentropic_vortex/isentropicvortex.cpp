...
inline a_real IsentropicVortexProblem::computeOmega(const std::array<a_real,2> r) const
{
	const a_real f = -0.5/(conf.sigma*conf.sigma*conf.clength*conf.clength)*(r[0]*r[0]+r[1]*r[1]);
	return conf.strength * std::exp(f);
}

inline std::array<a_real,2> IsentropicVortexProblem::computeVelocity(const std::array<a_real,2> r,
                                                                     const a_real omega) const
{
	...
	(const std::array<a_real,2> r, a_real *const u) const
{
	...
	const std::array<a_real,2> v = computeVelocity(r,omega);
