        ...
        for(ied = m->gnbface(); ied < m->gnaface(); ied++)
        {
            ...
            for(int ig = 0; ig < ng; ig++)      // iterate over gauss points
            {
                for(int i = 0; i < nvars; i++)
                {
                    ...
                    (*ufl)(ied,i) = u->get(ielem,i) + dudx->get(ielem,i)*(gx->get(ied,ig)-xi->get(ielem)) + dudy->get(ielem,i)*(gy->get(ied,ig)-yi->get(ielem));
                    (*ufr)(ied,i) = u->get(jelem,i) + dudx->get(jelem,i)*(gx->get(ied,ig)-xi->get(jelem)) + dudy->get(jelem,i)*(gy->get(ied,ig)-yi->get(jelem));
                }
            }
        }
