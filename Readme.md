...
To run the unit tests, run `ctest` in your `build-dir/tests` directory. The `tests` directory will have been generated by cmake upon building.
...
When running cases with implicit time stepping, remember to either use only one thread, or increase the number of asynchronous preconditioning sweeps (the last option in the control files) to 3 or 4 when using matrix storage type 'b' (recommended).
