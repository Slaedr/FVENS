...
#include "alimiter_parallel_cons.hpp"
...
public:
	...
	void convert_to_primitive_cell(Matrix<double>* u, Matrix<double>* uprim)
	{
		int iel;
	...
	void convert_to_primitive_bface(Matrix<double>* u, Matrix<double>* uprim)
	{
		int iel;
		for(iel = 0; iel < m->gnbface(); iel++)
		{
			(*uprim)(iel,0) = u->get(iel,0);	// density
			(*uprim)(iel,3) = (g-1)*(u->get(iel,3) - 0.5*(pow(u->get(iel,1),2)+pow(u->get(iel,2),2))/u->get(iel,0));	// pressure
			(*uprim)(iel,1) = u->get(iel,1)/u->get(iel,0);		// x-velocity
	...
	void convert_to_conserved_cell(Matrix<double>* upr, Matrix<double>* u)
	{
		int iel;
		double vmag2;
		for(iel = 0; iel < m->gnelem(); iel++)
	...
	void convert_to_conserved_face(Matrix<double>* upr, Matrix<double>* u)
	{
		int iel;
		double vmag2;
		for(iel = 0; iel < m->gnaface(); iel++)
		{
			(*u)(iel,0) = upr->get(iel,0);		// density
			vmag2 = upr->get(iel,1)*upr->get(iel,1) + upr->get(iel,2)*upr->get(iel,2);		// square of magnitude of velocity
			(*u)(iel,1) = upr->get(iel,1)*upr->get(iel,0);		// x-momentum per unit volume
	...
	void compute_ls_reconstruction(Matrix<double>* u)		// u should be a nelem x nvars matrix
	{
		...
		compute_ghostcell_states();		//  - CHECK
		
		//convert u to primitive variables
		convert_to_primitive_cell(u,u);
		convert_to_primitive_bface(&ug,&ug);
		
		(*uinf)(0,3) = (g-1)*(uinf->get(0,3) - 0.5*(pow(uinf->get(0,1),2)+pow(uinf->get(0,2),2))/uinf->get(0,0));	// pressure
		(*uinf)(0,1) = uinf->get(0,1)/uinf->get(0,0);		// x-velocity
		(*uinf)(0,2) = uinf->get(0,2)/uinf->get(0,0);		// y-velocity
		...
		convert_to_conserved_cell(u,u);
		convert_to_conserved_face(&ufl,&ufl);
		convert_to_conserved_face(&ufr,&ufr);
		
		double vmag2 = uinf->get(0,1)*uinf->get(0,1) + uinf->get(0,2)*uinf->get(0,2);		// square of magnitude of velocity
		(*uinf)(0,1) = uinf->get(0,1)*uinf->get(0,0);		// x-momentum per unit volume
		(*uinf)(0,2) = uinf->get(0,2)*uinf->get(0,0);		// y-momentum per unit volume
		(*uinf)(0,3) = uinf->get(0,3)/(g-1.0) + 0.5*uinf->get(0,0)*vmag2;		// energy per unit volume
